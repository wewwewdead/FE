                    ...
                    if (response.ok) {
                        ...
                        <div class="comment">
                                                    <div class="name-section">
                                                                ...
                                                                <a href="/user/${newComment}" class="profile_link author"><small>${newComment.user.firstname} ${newComment.user.lastname}:</small></a>
                                                            </div>
                                                    <p class="publishDate">
                                                    ...
                                                    <p class="content">${commentContent}</p>
                                                     ...
                                                     <div id="replies-${newComment.user.id}" class="replies-container hidden">
                                                    <div id="showmore-${newComment.user.id}>">
                                                            <button class='show-more-replies hidden button${newComment.user.id}' data-comment-id="${newComment.user.id}">Show More Replies</button>
                                                    </div>
                                                </div>
                                                
                                                              <div class="reply-input${newComment.user.id} replycontainer hidden" id="reply-input-${newComment.user.id}">
                                                                    <input type="text" class="reply-comment-input" placeholder="Write a reply...">
    
                                                                    <button class="submit-reply-btn" data-comment-id="${newComment.user.id}">Send</button>
                                                                </div>`
                ...
                try {
                    ...
                    //    console.log(`check${data.totalPages}`)
                           ...
                           data.replies.forEach(reply => {
                               ...
                               <div class="name-section">
                               <img id="newsfeed-profile-comment" src="${reply.users.profile_pictures || './uploads/default-profile.png'}">
                               <p>
                               <a href="/user/${reply.user_id}" class="profile_link author">
                               <small><span>${reply.users.firstname} ${reply.users.lastname}:</small></span></a>
                               </p>
                               </div>
                            ...
                            const replyHtml = `
                                <div class="reply">
                                    ...
                                    <p><a href="/user/${reply.user_id}" class="profile_link author">
                                        <small>${reply.users.firstname} ${reply.users.lastname}:</small></a></p>
                                    <p class="content_reply">${reply.content}</p>
                                    <p class="publishDate"><small>${new Date(reply.created_at).toLocaleString()}</small></p>
                                </div>`;
                            repliesContainer.insertAdjacentHTML('beforeend', replyHtml);
                ...
                if (response.ok) {
                    ...
                    const replyHTML = `
                        <div class="reply">
                            ...
                            <p class="content_reply">${replyContent}</p>
                            <p class="publishDate">
                                <small>${new Date(Date.now()).toLocaleString()}</small>
                            </p>
                        </div>
